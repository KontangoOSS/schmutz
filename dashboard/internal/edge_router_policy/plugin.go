...
func (p *Plugin) Register(router *bff.Router) {
	...
	router.Handle("GET /api/edge-router-policies", listAll(z, base))
	router.Handle("POST /api/edge-router-policies", create(router, base))
	router.Handle("PATCH /api/edge-router-policies/{id}", patch(z, base))
	router.Handle("DELETE /api/edge-router-policies/{id}", del(z, base))
}
