...
func (p *Plugin) Name() string        { return "transit-router" }
func (p *Plugin) Description() string { return "CRUD for Ziti transit routers" }

func (p *Plugin) Register(router *bff.Router) {
	z := router.Ziti
	base := "/edge/management/v1/transit-routers"
	router.Handle("GET /api/transit-routers", listAll(z, base))
	router.Handle("POST /api/transit-routers", create(router, base))
	router.Handle("PATCH /api/transit-routers/{id}", patch(z, base))
	router.Handle("DELETE /api/transit-routers/{id}", del(z, base))
}
